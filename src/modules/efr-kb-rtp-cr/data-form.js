...
export class DataForm {
  ...
  itemChanged(newValue, oldValue) {
    ...
    if (selectedSupplier) {
      var _data = this.data.items.find(
        (item) => item.code === selectedSupplier.code
      );
      if ($data || !_data) {
  ...
  makeTotal(items) {
    ...
    if (Object.getOwnPropertyNames(items).length > 0) {
      for (var i = 0; i < items.length; i++) {
        ...
        this.sumPrice +=
          parseInt(items[i].sendquantity) * items[i].item.domesticSale;
      }
    }
  }
  ...
  async attached() {
    ...
    this.service.getDestinations().then((result) => {
    ...
    this.service.getSources().then((result) => {
